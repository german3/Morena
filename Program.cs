...
using OpenAI;
using Morena.Components;
using Morena.Services;
using Morena.Services.Ingestion;
...
{
    Console.WriteLine("Warning: GitHubModels:Token is missing. Registering Mock Services.");
...
else
{
    ...
    chatClient = ghModelsClient.GetChatClient("gpt-4o-mini").AsIChatClient();
    embeddingGenerator = ghModelsClient.GetEmbeddingClient("text-embedding-3-small").AsIEmbeddingGenerator();
}
